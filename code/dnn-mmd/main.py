...
#
# # x = torch.FloatTensor(np.vstack((X_train0, X_train1[:100, :])))
# # y = torch.FloatTensor(np.vstack((Y_train0, Y_train1[:100, :])))
...
#
# # print(mmd_loss(net(x_all)[1], net(x1)[1]))
#
# # torch.save(net, "my_model_mmd.pkl")
...
# #
# # x_test1 = torch.FloatTensor(X_train1)
# # y_test1 = torch.FloatTensor(Y_train1)
# # x_test1 = Variable(x_test1)
# # y_test1 = Variable(y_test1)
# #
# # predict1 = net(x_test1)[0]
...
if __name__ == '__main__':
    ...
    max_X2 = np.amax(source_X, axis=0)
    if 0 in max_X2:
        max_X2[max_X2 == 0] = 1
    source_X = np.divide(source_X, max_X2)

    max_Y2 = np.max(source_Y) / 100
    if max_Y2 == 0:
        max_Y2 = 1
    source_Y = np.divide(source_Y, max_Y2)

    x_all = torch.FloatTensor(source_X)
    x_all = Variable(x_all)
    ...
    for N in range(1, 21):
        ...
        for i in range(5):
            ...
            for i in range(epochs):
                ...
                loss = loss_func(prediction, y1) + mmd_loss(net(x_all)[1], net(x1)[1])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                print(i+1, "loss:", loss.data.item())
